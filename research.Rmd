...
DIFF
# by col
y <- mat[2:50,2:50] %>% as.vector()
x <- rep(NA,49)
for (i in 1:10){
  x.i <- DIFF[,(49*(i-1)+1):(49*i)]
  x.i <- as.vector(x.i)
  x <- cbind(x, x.i)
}
x[,1] <- y
colnames(x) <- c("y", paste0("x.",1:10))
df <- data.frame(x)
# fit glm
fit=glm(y~.,family = binomial, data = df)
summary(fit)
#plf
plf <- function(DIFF, param){
  if (length(param) == 10){
  sum ( log ( 1 + exp (DIFF*param)) )}
  else {warning("param length wrong, not = 10")}
}
